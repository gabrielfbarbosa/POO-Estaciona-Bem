...
import estacionabem.TabelaValores;
...
public class OperacaoEstacionar {
    ...
    public void estacionar(ArrayList<Vaga> vagas ,ArrayList<Cliente> clientes, ArrayList<Ticket> ticketsHoristas, ArrayList<Ticket> ticketsMensais, TabelaValores[] dias){
        Vaga vaga;
    ...
    /*2 - Retirar================================*/
    public void retirar(ArrayList<Vaga> vagas, ArrayList<Cliente> clientes, ArrayList<Ticket> ticketsHoristas,  ArrayList<Ticket> ticketsMensais, ArrayList<Ticket> ticketsFinalizados ,TabelaValores[] dias){
        
        String placa = JOptionPane.showInputDialog(null, "Digite a placa do veiculo que irá retirar:", "Placa X");
        
        Ticket ticketH = buscaTicket(ticketsHoristas, placa);
        Ticket ticketM = buscaTicket(ticketsMensais, placa);
        
        if(ticketH == null && ticketM == null){
            JOptionPane.showMessageDialog(null, "Ticket não encontrado!", "Estaciona Bem", JOptionPane.ERROR_MESSAGE);
        } else if(ticketH != null){
            
            ticketH.finalizaTicket();
            ticketsFinalizados.add(ticketH);
            
            Vaga vaga = opVaga.buscaVaga(vagas, ticketH.getVaga().getNumero(), ticketH.getVaga().getRua());
            vaga.setDisponibilidade("disponivel");
            
            JOptionPane.showMessageDialog(null, ticketH.toString());
        } else{
            ticketM.finalizaTicket();
            ticketsFinalizados.add(ticketM);
            
            Vaga vaga = opVaga.buscaVaga(vagas, ticketM.getVaga().getNumero(), ticketM.getVaga().getRua());
            vaga.setDisponibilidade("disponivel");
            
            JOptionPane.showMessageDialog(null, ticketM.toString());
        }
    }
